...
import { deleteToDo, editTodo } from '../Reducers/reducers';
  ...
  return <div>
      {
          isEditing ?
            <div className='form'>
                <h2>Update your task from here</h2>
            ...
            todoList.map(({id, content})=> {
                return <li className='grid' key={id}>
                    ...
                    <span className='todo-action'>
                      <button className="close btn btn-danger" onClick={() => dispatch(deleteToDo({id}))}>Remove</button>
                      <button className="edit btn btn-primary" onClick={() =>onEditToggle(id, content)} >Edit</button>
                    </span>
                </li>
            }) 
          }
